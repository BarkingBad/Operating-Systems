...
int in(void *const a, void *const pbase, size_t total_elems, size_t size, __compar_fn_t cmp)
{
    ...
    if (total_elems > 0)
    {
        ...
        {
            if ((*cmp)(a, (void *)(base_ptr + i * size)) == 0)
                return i;
        }
    }
    return -1;
...
void handle_connection(int socket)
{
    ...
    if (client == -1)
        raise_error(" Could not accept new client");
...
void handle_message(int socket)
{
    ...
    if (read(socket, &message_size, LEN_SIZE) != LEN_SIZE)
        raise_error(" Could not read message size\n");
    ...
    case PONG:
    {
        ...
        if (i >= 0)
            clients[i].active_counter = clients[i].active_counter == 0 ? 0 : clients[i].active_counter - 1;
        pthread_mutex_unlock(&mutex);
        break;
    }
    default:
...
void delete_socket(int socket)
{
    ...
    if (shutdown(socket, SHUT_RDWR) == -1)
        raise_error(" Could not shutdown client's socket\n");

    if (close(socket) == -1)
        raise_error(" Could not close client's socket\n");
}
